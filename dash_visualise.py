    ...
    def initalise_visualiser(self):
        ...
        def update_scatter(velo_checklist, world_system, zlims, alpha):
            ...
            fig = px.scatter_3d(dff, x=dff[coords[0]], y=dff[coords[1]], z=dff.Velocity,
                    ...
                    custom_data=[dff.x, dff.y, dff.component], range_x=xlims, range_y=ylims, range_z=zlims)
        ...
        def update_2d_image(map_type, colour_range, clickData, zoom, world_system):
            ...
            figh = px.imshow(data_2d[yl:yu, xl:xu], origin='lower', aspect='equal',
                             zmin=colour_range[0], zmax=colour_range[1],
                             template="simple_white", x=x[xl:xu], y=y[yl:yu],
                             labels=dict(
                                 x=config.coord_types[world_system][0],
                                 y=config.coord_types[world_system][1]
                                 )
                             )
...
def get_model_for_each_found_component(velocity, models, y, x, tex_label='tex', tau_label='tau', peak_v_label='v', sigma_label='sigma'):
    """
    ...
    single_model = [make_maps.hyperfine_structure_all(velocity, #real_data_obj.all_data_3d['velocity'],
                                               ...
                                               return_hyperfine_components=False) for i in range(len(models[tex_label]))]
