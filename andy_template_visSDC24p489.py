...
>If the error _reload-hash appears. Just wait a couple of seconds or refresh page
...
velo_start     = [45, 55] #Set to None if not wanted, or exlcude from function call
...
single_model_fits = get_data.all_fit_param_cubes(component_filename, ['sigma_%d', 'tau_%d', 'tex_%d', 'v_%d'], err_suffix=None)#['_err_down', '_err_up'])
...
vis = dash_visualise.Visualiser(
    ...
    data_2d_obj=data_2d_obj,       #2d data map
    real_data_obj=real_data_obj,   #datacubes
